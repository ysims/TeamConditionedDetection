...
class DetectionMetrics:
    ...
    def update(self, preds: list[dict[str, torch.Tensor]], targets: list[dict[str, torch.Tensor]]) -> None:
        ...
        for pred, target in zip(preds, targets):
            ...
            tp, fp, fn = self._match_one_image(
                pred["boxes"][keep], pred["scores"][keep], pred["labels"][keep], target["boxes"], target["labels"]
            )
            self._tp += tp
            self._fp += fp
            self._fn += fn

    def _match_one_image(self, pred_boxes, pred_scores, pred_labels, gt_boxes, gt_labels) -> tuple[int, int, int]:
        if gt_boxes.shape[0] == 0:
            return 0, pred_boxes.shape[0], 0
        if pred_boxes.shape[0] == 0:
            return 0, 0, gt_boxes.shape[0]
        # Highest-confidence predictions get first claim on a GT box (standard
        # COCO-style greedy matching) - without this, a low-confidence duplicate
        # that happens to come first in `pred_boxes` could steal a GT box from a
        # higher-confidence detection of the same object, undercounting TPs.
        # Doesn't change this project's own reported numbers (every image here
        # has exactly one GT box, so there's never more than one candidate to
        # arbitrate between), but matters for any multi-instance-per-image use.
